...
def elu_activate_map(x):
    return F.elu(x) + 1


class LinearAttention(nn.Module):
    def __init__(self, dim, attn_mode, num_heads, qkv_bias=False, eps=1e-6):
        super().__init__()
        ...
        self.eps = eps
        self.elu_activate_map = elu_activate_map
    ...
    def forward(self, x):
        ...
        Q = self.elu_activate_map(query)
        K = self.elu_activate_map(key)
...
class LinearTransformerEncodeBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.linear_attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        
        return x


class LinearTransformerEncoderLayers(nn.Module):
    def __init__(self,  dim, layer_attn_modes, num_heads, qkv_bias=False, 
                mlp_ratio=4, drop_path=0., norm_layer=nn.LayerNorm,
                mlp_act_layer=nn.GELU, mlp_drop=0.
    ):
        super().__init__()
        self.LTEL = nn.ModuleList(
            [
                LinearTransformerEncodeBlock(dim, layer_attn_mode, num_heads, qkv_bias, mlp_ratio, drop_path, norm_layer, mlp_act_layer, mlp_drop) for layer_attn_mode in layer_attn_modes
            ]
        )
        
    
    def forward(self, x):
        for i, transformer_block in enumerate(self.LTEL):
